...
def find_channel_index(image, channel_name):
    ...
    for channel in image.getChannels():
        ...
        if channel_name in name:
            ...
            break
        count = count + 1
    # TODO check map annotation for some IDR data
    return found, index
...
def get_valid_region(image, x, y, w, h):
    ...
    if x < 0 or x >= size_x: return None
    if y < 0 or y >= size_y: return None
    if w < 0 or w > size_x: return None
    if h < 0 or h > size_y: return None
    if x + w > size_x: return None
    if y + h > size_y: return None
